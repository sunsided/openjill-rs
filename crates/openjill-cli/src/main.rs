...
fn write_dump(request: DumpRequest) -> Result<()> {
    ...
    if request.kind == DumpFrameworkKind::Sha {
        ...
        let atlas_indexed_file = output_dir.join(SHA_ATLAS_INDEXED_FILE);
        let atlas_rgb_file = output_dir.join(SHA_ATLAS_RGB_FILE);
        ...
        write_binary_file(
            &atlas_indexed_file,
            &sha_dump.atlas_indexed_png,
            request.force,
        )?;
        write_binary_file(&atlas_rgb_file, &sha_dump.atlas_rgb_png, request.force)?;
    } else {
...
/// including JSON metadata and both atlas PNG payloads.
struct ShaDumpOutput {
    ...
    /// Encoded 24-bit RGB PNG produced by expanding indexed pixels through the Jill VGA palette.
    atlas_rgb_png: Vec<u8>,
}
...
fn sha_dump_output(data_dir: &Path, episode: &Episode) -> Result<ShaDumpOutput> {
    ...
    let atlas_rgb_png = encode_rgb_png(&atlas, &palette)?;
    ...
    let json = serde_json::json!({
        ...
        "atlas_files": [
            ...
            {
                "file_name": SHA_ATLAS_RGB_FILE,
                "pixel_format": "rgb8",
                "palette_source": "jill_vga",
                "width": atlas.width,
                "height": atlas.height,
                "tile_padding": SHA_ATLAS_TILE_PADDING,
            },
...
/// Encodes row-major indexed atlas pixels into a 24-bit RGB PNG byte vector by
/// expanding each pixel through `palette`. Used for human-viewable atlas dumps.
fn encode_rgb_png(atlas: &ShaAtlasImage, palette: &Palette) -> Result<Vec<u8>> {
    let width = u32::try_from(atlas.width)
        .map_err(|error| anyhow::anyhow!("atlas width does not fit u32: {error}"))?;
    let height = u32::try_from(atlas.height)
        .map_err(|error| anyhow::anyhow!("atlas height does not fit u32: {error}"))?;
    let entries = palette.entries();
    let mut rgb = Vec::with_capacity(atlas.pixels.len() * 3);
...
mod tests {
    ...
    fn dump_writes_sha_payload_metadata_and_indexed_atlas() {
        ...
        check!(metadata["atlas_files"][1]["file_name"] == Value::from(SHA_ATLAS_RGB_FILE));
        check!(metadata["atlas_files"][1]["pixel_format"] == Value::from("rgb8"));
        check!(metadata["atlas_files"][1]["palette_source"] == Value::from("jill_vga"));
        check!(metadata["atlas_files"][1]["tile_padding"] == Value::from(SHA_ATLAS_TILE_PADDING));
        check!(metadata["atlas_files"][1]["width"] == Value::from(5));
        check!(metadata["atlas_files"][1]["height"] == Value::from(6));
        ...
        let rgb_path = output.join(SHA_ATLAS_RGB_FILE);
        check!(rgb_path.is_file());
        let (rgb_width, rgb_height, rgb_pixels) = read_rgb_png(&rgb_path);
        check!(rgb_width == 5);
        check!(rgb_height == 6);
        check!(rgb_pixels.len() == atlas_pixels.len() * 3);
        let palette = openjill_core::Palette::jill_vga();
        for (pixel_index, &indexed) in atlas_pixels.iter().enumerate() {
            let [r, g, b] = palette.entries()[indexed as usize];
            check!(rgb_pixels[pixel_index * 3] == r);
            check!(rgb_pixels[pixel_index * 3 + 1] == g);
            check!(rgb_pixels[pixel_index * 3 + 2] == b);
        }
    }
    ...
    /// Unit under test: multi-artifact SHA dump overwrite safeguards for the RGB atlas.
    ///
    /// Preconditions: the selected SHA output directory already contains an
    /// RGB atlas artifact while metadata and the indexed atlas do not exist yet.
    ///
    /// Invariants asserted: the command refuses the request before writing
    /// metadata, so a failed no-force SHA dump cannot leave partial output even
    /// when only the RGB sibling pre-exists.
    #[test]
    fn dump_sha_refuses_existing_rgb_atlas_before_writing_metadata() {
        let temp_dir = TempDirGuard::new("openjill-cli-dump-sha-partial-overwrite-rgb");
        write_valid_episode_one_fixture(temp_dir.path());
        fs::write(temp_dir.path().join("JILL1.SHA"), valid_sha_dump_bytes())
            .expect("write custom JILL1.SHA");
        let output = temp_dir.path().join("sha-dump");
        fs::create_dir_all(&output).expect("create output directory");
        fs::write(output.join(SHA_ATLAS_RGB_FILE), b"existing rgb atlas")
        ...
        let Err(error) = dispatch(cli.command) else {
            panic!("dump should refuse to overwrite existing rgb atlas");
        };

        check!(error.to_string().contains("--force"));
        check!(!output.join("metadata.json").exists());
        check!(!output.join(SHA_ATLAS_INDEXED_FILE).exists());
    }
    ...
    /// Reads an RGB PNG file and returns dimensions plus row-major interleaved RGB bytes.
    fn read_rgb_png(path: &Path) -> (usize, usize, Vec<u8>) {
        let png_bytes = fs::read(path).expect("read atlas png");
        let cursor = Cursor::new(png_bytes);
        let decoder = Decoder::new(cursor);
        let mut reader = decoder.read_info().expect("decode atlas header");
        ...
        let output = reader
            .next_frame(&mut decode_buffer)
            .expect("decode atlas frame");
        check!(output.color_type == png::ColorType::Rgb);
        check!(output.bit_depth == png::BitDepth::Eight);
        let used = output.buffer_size();
        (
            output.width as usize,
