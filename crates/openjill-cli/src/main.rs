...
/// Parses one SHA input source and wraps parser failures with file context.
...
/// Complete deterministic output generated by `dump sha`.
...
/// One packed atlas image ready to be encoded for SHA dumps.
...
/// Returns the deterministic column count used by row-major atlas packing.
...
/// Encodes grayscale indexed atlas pixels into an 8-bit PNG image.
...
/// Writes bytes through a temporary file and atomically renames them into place.
